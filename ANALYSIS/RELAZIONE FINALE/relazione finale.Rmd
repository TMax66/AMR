...
title: ''
author: ''
header-includes: \usepackage[italian]{babel} \usepackage{setspace}\doublespacing 
output:
  pdf_document:
    toc: yes
    number_sections: yes
...
### Metodi statistici
**Prevalenza animali portatori di ceppi resistenti**  Sulla base dei profili di resistenza dei singoli ceppi si è provveduto a classificare i ceppi in Suscettibili (ceppi sensibili a tutti gli antibiotici del panel) e Resistenti (ceppi sensibili ad almeno un  antibiotico del panel) e quindi a identificare i campioni di provenenienza dei ceppi classificando come campione di feci "Resistente" feci  portatori di almeno un ceppo resistente ad almeno un antibiotico del panel indagato. La prevalenza complessiva e specifica per specie dei campioni Resistenti  è stata stimata mediante il seguente modello:

$$R_i \sim \text Bernoulli(P)$$
$$\text logit(p) = \alpha + \beta*Specie $$ $$\alpha \sim Normal (0, 2.5)$$ $$\beta \sim Normal (0,2.5)$$ 
da cui deriva che $R_i$, cioè la resistenza ad almeno un antibiotico in almeno un ceppo isolato da un campione di feci si distribuisce secondo una distribuzione Bernoulli con probabilità P. P è modellato via trasformazione logistica da un modello lineare con parametri $\alpha$ e $\beta$. Dove $\alpha$ rappresenta l'intercetta e $\beta$ l'effetto della specie. Per questi due parametri si assume una distribuzione a priori debolmente informativa, scelta per ottimizzare il campionamento delle distribuzioni a posteriori ottenuta mediante l'algoritmo Hamiltonian Monte Carlo (HMC), utilizzando quattro catene random com 2000 iterazioni di cui 1000 di warnup. Per il campionamento è stato utilizzando la funzione stan_glm() del pacchetto rstanarm, da cui sono state estratti gli effetti marginali ottenendo le distribuzioni a posteriore del logit della prevalenza trasformato in probabilità mediante la funzione inversa:
$$ P_i = \frac{\exp(\alpha + \beta *x_i)}{1+\exp(\alpha + \beta *x_i)} $$ La prevalenza complessiva di campioni portatori di ceppi resistenti ad almeno un antibiotico è stata stimata utilizzando una versione del modello precendente con solo l'intercetta ( modello nullo). Le stime della prevelenza sono quindi espresse come mediana delle distribuzione a posteriori di P e i rispettivi intervalli di credibilità calcolati al 95% utilizzando i percentili 2.5 e 97.5 delle distribuzioni a posteriori di P, e rappresentati graficamente mediante density-ridges plot. 
...
Dove X rappresenta la matrice dei predittori e $\alpha_{\text{Comune}}$ definisce la variabile con intercetta variabile.  Tutte le variabili quantitative predittorie (Urbanizzazione e Pascolo) sono state standardizzate in accordo a quanto suggerisce Gelman, sia per una maggior interpretabilità dei coefficienti sia per una migliore performance del motore HCMCM che per una miglior utilizzo delle prior del modello. Si è proceduto ad adattare diversi modelli utilizzando i diversi predittori esplorando termini d'interazione ragionevoli dal punto di vista epidemiologico. Il confronto tra i i diversi modelli mediante le metriche dei criteri d'informazione LOO ( Leave-One-Out Cross Validation) ha permesso di selezionare il modello con maggior potere informativo. Le stime dei parametri del modello sono state ottenute mediante campionamento con l'algoritmo Hamiltonian Monte Carlo utilizzando quattro catene, con 4000 iterazioni di cui 2000 di warmup ( escluse dopo il campionamento). Per questo modello è stato utilizzato il pacchetto brms come interfaccia del linguaggio STAN in ambiente R. 

**Prevalenza di ceppi resistenti e profili fenotipici di resistenza ai differenti antibiotici**.   Sulla base dei profili di resistenza dei singoli ceppi si è provveduto a classificare i ceppi in * Suscettibili  (ceppi sensibili a tutti gli antibiotici del panel)
...
Sulla base delle resistenze ai differenti antibiotici del panel i ceppi sono stati caratterizzati per il loro profilo di resistenza indicato da una stringa costituita dall'abbreviazione di tre lettere degli antibiotici resistenti. Ad esempio la stringa AMP-TET-COL indica un ceppo batterico con fenotipo di resistenza a Ampicillina, Tetraciclina, e Colistina. I differenti ceppi sono stati quindi aggregati per fenotipo e per gruppo-specie di provenienza dei ceppi e analizzati mediante metodi di analisi della biodiversità calcolando quattro misure di diversità legate alle misure di entropia generalizzata di Rényi. Questo approccio è  descritto da Mathe et al (rif) dove l'esponenziale dell'entropia di Rényi ($\text{D}\alpha$) stima il numero effettivo di specie (in questo caso di fenotipi di resistenza) e $\alpha$ rappresenta un parametro di scala, lungo il quale la richezza del profilo e l'abbondanza relativa dei profili sono pesate in modo differente. I quattro indici di diversità che sono stati confrontati tra i comparti rappresentati dai gruppi-specie si trovano a diversi livelli di $\alpha$: a D0, profile richness (PR), rappresenta una semplice conta dei differenti profili di resistenza che ignora la relativa abbondanza e quindi considera profili rari e comuni allo stesso modo; a log(D1), Shannon entropy (SEn) rappresenta la probabilità che due ceppi estratti a caso hanno lo stesso profilo di resistenza; a 1/D2 Simposon diversity(SD) è l'abbondanza relativa di ogni profilo e a 1/D$\infty$ Berger-Parker diversity(BP) che rappresenta la proporzione dei profili più comuni nel campione.
...
### Fauna selvatica
Sono stati raccolti complessivamente 671 campioni di feci da 33 differenti specie di fauna selvatica.  Le specie più frequentemente campionate sono il Capriolo (Capreolus capreolus) con 201 campioni ( 27.6%),  la Cornacchia grigia (Corvus cornix) con 133 (18.2%), il Cervo (Cervus elaphus) ,  con 83 campioni (11.4%), il Cinghiale (Sus scrofa) con 78 campioni (10.7%), il Camoscio (Rupicapra rupicapra) 49 campioni (6.7%) e il Muflone (Ovis aries musimon) 44 campioni (6.0%) che rappresentano complessivamente l'80% dei campioni esaminati. Il restante 20% è distribuito su un totale di 27 specie differenti (tab \ref{tab:tab2})

-distribuzione territoriale dei gruppi specie (usare ARM-istat IDcamp filtrati....)
...
## PREVALENZA DI CEPPI ANTIBIOTICO-RESISTENTI (E MULTI-RESISTENTI....verificare) NELLA FAUNA SELVATICA
...
\hline
CERVIDI & CAPRIOLO & 191 & 28.46\\
\hline
CERVIDI & CERVO & 80 & 11.92\\
\hline
CERVIDI & DAINO & 1 & 0.15\\
\hline
BOVIDI & CAMOSCIO & 46 & 6.86\\
\hline
BOVIDI & MUFLONE & 43 & 6.41\\
\hline
BOVIDI & STAMBECCO & 6 & 0.89\\
\hline
CARNIVORI & TASSO & 4 & 0.60\\
\hline
CARNIVORI & VOLPE & 16 & 2.38\\
\hline
SUIDI & CINGHIALE & 76 & 11.33\\
\hline
LEPRE & LEPRE & 3 & 0.45\\
\hline
CORVIDI & CORNACCHIA & 122 & 18.18\\
\hline
